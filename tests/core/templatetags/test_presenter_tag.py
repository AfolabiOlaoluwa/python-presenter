...
class TestPresenterTag:
    def test_present_object_with_presenter_class(self, project, mock_present_function):
        """Test present_object function with a specific presenter class"""
        present_object(context, project, ProjectPresenter)

        mock_present_function.assert_called_once_with(project, ProjectPresenter)

    def test_present_object_without_presenter_class(self, project, mock_present_function):
        """Test present_object function without specifying a presenter class"""
        present_object(context, project)

        mock_present_function.assert_called_once_with(project, None)

    @pytest.mark.django_db
    def test_django_template_tag_registration(self, project, mock_present_function):
        """Test that the template tag is properly registered in Django"""
        @register.simple_tag(takes_context=True)
        def present_object(context, obj, as_var=None):
            """A mock implementation of present_object for testing."""
            presenter_class = mock_present_function.return_value
            presenter_instance = presenter_class(obj)
            if as_var:
                context[as_var] = presenter_instance
        ...
        context = Context({'project': project})
        rendered_output = templ.render(context)
        ...
        assert 'presenter_tag' in engine.template_libraries, "Template tag library 'presenter_tag' is not registered."
        assert hasattr(template_tag_library, 'tags')
        assert 'present_object' in template_tag_library.tags, "Template tag 'present_object' is not registered."
        assert rendered_output.strip() == expected_output.strip(), f"Rendered output:\n{rendered_output}"
    ...
    def test_present_object_integration(self, project):
        """Test actual integration with the presenter system"""
        presented = present_object(context, project, ProjectPresenter)
        ...
        assert presented.property_unit_type() == "APARTMENT"  # Note: uppercase due to presenter logic
    ...
    def test_presenter_labels(self, project, attribute):
    ...
    @patch('python_presenter.core.presenters.presenter_helper.present')
    def test_present_object_error_handling(self, mock_present, project):
        """Test error handling in present_object function"""
        # Simulate an error in the present function
        mock_present.side_effect = Exception("Presentation error")
